...
const Config = require('../config');
    ...
    Ktb.addCommand({pattern: 'sendi$', fromMe: true, desc: 'Download status from wa'}, (async (message, match) => {    
        ...
        ffmpeg(location)
            .save('status.mp4')
            .on('end', async () => {
                await message.client.sendMessage(message.jid,fs.readFileSync('status.mp4'), MessageType.video , {caption: '```WHATSAPP STATUS  🍃```'  , mimetype: Mimetype.mp4, quoted: message.data,thumbnail: White.tm_b});
            });
        return 
    }));
   ...
   Ktb.addCommand({pattern: 'sendv$', fromMe: true, desc: 'Download status from wa'}, (async (message, match) => {    
        ...
        if (message.reply_message.video === true && message.reply_message.video) {
            ...
            .save('output.mp4')
            .on('end', async () => {
                await message.client.sendMessage(message.jid, fs.readFileSync('output.mp4'), MessageType.video ,{caption: '```WHATSAPP STATUS  🍃```'  , mimetype: Mimetype.mp4, quoted: message.data,thumbnail: White.tm_b});
            });
        return 
        }
